...
public class AjaxTestController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * ajax통신시 데이터전송에 가장 많이 사용되는 포멧형식중 하나
		 * 
		 * JSONArray
		 * [value,value,value...]
		 * 
		...
		*/
		...
		list.add(new Member(3, "박개똥", "01011112222"));
		list.add(new Member(4, "이개똥", "01015426215"));
		list.add(new Member(5, "서개똥", "01011112222"));
		/*
		JSONArray jArr = new JSONArray();
		for(Member m : list) {
			JSONObject jobj = new JSONObject();
			jobj.put("userNO", m.getUserNo());
			jobj.put("userName", m.getUserNo());
			jobj.put("phne", m.getPhone());
